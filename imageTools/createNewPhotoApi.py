...
def process_image():
    ...
    data = request.form
    border_size_ratio = float(data.get('border_size_ratio', 0.07))
    text_area_ratio = float(data.get('text_area_ratio', 0.09))
    text = data.get('text', '')
    base_path = os.path.dirname(os.path.abspath(__file__))
    font_path = data.get('font_path', base_path + "/font/Konatu-2.ttf")
    blur_radius = int(data.get('blur_radius', 300))
    text_alignment = data.get('text_alignment', 'center')
    font_color = data.get('font_color', 'white')
    shadow_offset = tuple(map(int, data.get('shadow_offset', '40,40').split(',')))
    shadow_color = tuple(map(int, data.get('shadow_color', '0,0,0,128').split(',')))
    corner_radius_ratio = float(data.get('corner_radius_ratio', 0.05))
    font_size = int(data.get('font_size', 180))
    image_text_margin = int(data.get('image_text_margin', 80))
    line_spacing = int(data.get('line_spacing', 10))
    output_prefix = os.path.splitext(image_path)[0]
    output_extension = os.path.splitext(image_path)[1]
    output_path = f'./{output_prefix}{str(time.time()).split(".")[0]}{output_extension}'
